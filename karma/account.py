...
class Account(dict):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises karma.exceptions.AccountDoesNotExistsException: if account does not exist
        ...
        .. code-block:: python
            ...
            from karma.account import Account
    ...
    def refresh(self):
        ...
        if re.match("^1\.2\.[0-9]*$", self.name):
            account = self.karma.rpc.get_objects([self.name])[0]
        else:
            account = self.karma.rpc.lookup_account_names([self.name])[0]
        if not account:
            raise AccountDoesNotExistsException(self.name)

        if self.full:
            account = self.karma.rpc.get_full_accounts([account["id"]], False)[0][1]
    ...
    def balances(self):
        """ List balances of an account. This call returns instances of
            :class:`karma.amount.Amount`.
        """
        balances = self.karma.rpc.get_account_balances(self["id"], [])
        return [
            Amount(b, karma_instance=self.karma)
    ...
    def history(
        ...
        mostrecent = self.karma.rpc.get_account_history(
            self["id"],
            "1.11.{}".format(0),
            1,
        ...
        while True:
            # RPC call
            txs = self.karma.rpc.get_account_history(
                self["id"],
                "1.11.{}".format(last),
                _limit,
                "1.11.{}".format(first - 1),
    ...
    def upgrade(self):
        return self.karma.upgrade_account(account=self)


class AccountUpdate(dict):
    """ This purpose of this class is to keep track of account updates
        as they are pushed through by :class:`karma.notify.Notify`.
    ...
    def account(self):
        """ In oder to obtain the actual
            :class:`karma.account.Account` from this class, you can
