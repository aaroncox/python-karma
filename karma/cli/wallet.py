...
def changewalletpassphrase(ctx, new_password):
    ...
    """
    ctx.karma.wallet.changePassphrase(new_password)
    ...
    if not key:
        while True:
            ...
            if not key or key == "exit":
                break
            try:
                ctx.karma.wallet.addPrivateKey(key)
        ...
        for k in key:
            try:
                ctx.karma.wallet.addPrivateKey(k)
            except Exception as e:
                click.echo(str(e))

    installedKeys = ctx.karma.wallet.getPublicKeys()
    if len(installedKeys) == 1:
        name = ctx.karma.wallet.getAccountFromPublicKey(installedKeys[0])
        account = Account(name, karma_instance=ctx.karma)
        click.echo("=" * 30)
...
def delkey(ctx, pubkeys):
    ...
    if click.confirm(
        ...
        for pub in pubkeys:
            ctx.karma.wallet.removePrivateKeyFromPublicKey(pub)
...
def getkey(ctx, pubkey):
    ...
    """
    click.echo(ctx.karma.wallet.getPrivateKeyForPublicKey(pubkey))
...
def listkeys(ctx):
    ...
    t.align = "l"
    for key in ctx.karma.wallet.getPublicKeys():
        t.add_row([key])
    click.echo(t)
...
def listaccounts(ctx):
    ...
    t.align = "l"
    for account in ctx.karma.wallet.getAccounts():
        t.add_row([
            account["name"] or "n/a",
            account["type"] or "n/a",
            account["pubkey"]
