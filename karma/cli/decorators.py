...
from karma.instance import set_shared_karma_instance
...
def offlineChain(f):
    ...
    def new_func(ctx, *args, **kwargs):
        ...
        ctx.karma = karma(**ctx.obj)
        set_shared_karma_instance(ctx.karma)
        return ctx.invoke(f, *args, **kwargs)
    return update_wrapper(new_func, f)
