...
def allow(ctx, foreign_account, permission, weight, threshold, account):
    ...
    if not foreign_account:
        from karmabase.account import PasswordKey
        pwd = click.prompt(
...
def history(ctx, account, limit, type, csv, exclude, raw):
    ...
    """
    from karmabase.operations import getOperationNameForId
    ...
    for a in account:
        account = Account(a, karma_instance=ctx.karma)
...
def newaccount(ctx, accountname, account, password):
    ...
    """
    pprint(ctx.karma.create_account(
        accountname,
        registrar=account,
        password=password,
    ))
...
def upgrade(ctx, account):
    pprint(ctx.karma.upgrade_account(account))
