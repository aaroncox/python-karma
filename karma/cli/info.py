...
def info(ctx, objects):
    ...
    if not objects:
        ...
        t.align = "l"
        info = ctx.karma.rpc.get_dynamic_global_properties()
        for key in info:
    ...
    for obj in objects:
        # Block
        if re.match("^[0-9]*$", obj):
            block = Block(obj, karma_instance=ctx.karma)
            if block:
                t = PrettyTable(["Key", "Value"])
                t.align = "l"
        ...
        # Object Id
            ...
            data = ctx.karma.rpc.get_object(obj)
            if data:
                t = PrettyTable(["Key", "Value"])
                t.align = "l"
        ...
        # Public Key
        elif re.match("^KARMA.{48,55}$", obj):
            account = ctx.karma.wallet.getAccountFromPublicKey(obj)
            if account:
