...
class Witness(dict):
    ...
    def refresh(self):
        ...
        if len(parts) == 3:
            ...
            if int(b) == 6:
                witness = self.karma.rpc.get_object(self.witness)
            else:
                witness = self.karma.rpc.get_witness_by_account(self.witness)
        else:
            account = Account(self.witness)
            witness = self.karma.rpc.get_witness_by_account(account["id"])
        if not witness:
