...
from karmabase.objects import Operation
from karmabase.account import PrivateKey, PublicKey
from karmabase.signedtransactions import Signed_Transaction
...
class TransactionBuilder(dict):
    ...
    def appendSigner(self, account, permission):
        ...
        account = Account(account, karma_instance=self.karma)
        ...
        def fetchkeys(account, level=0):
            ...
            for authority in account[permission]["key_auths"]:
                wif = self.karma.wallet.getPrivateKeyForPublicKey(authority[0])
                if wif:
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(authority[0], karma_instance=self.karma)
    ...
    def constructTx(self):
        ...
        if self.karma.proposer:
            ...
            ops = operations.Proposal_create(**{
                ...
                "expiration_time": transactions.formatTimeFromNow(
                    self.karma.proposal_expiration),
        ...
        ops = transactions.addRequiredFees(self.karma.rpc, ops)
        expiration = transactions.formatTimeFromNow(self.karma.expiration)
        ref_block_num, ref_block_prefix = transactions.getBlockParams(self.karma.rpc)
    ...
    def sign(self):
        ...
        # presented wrongly!
        if self.karma.rpc:
            operations.default_prefix = self.karma.rpc.chain_params["prefix"]
        ...
        signedtx.sign(self.wifs, chain=self.karma.rpc.chain_params)
    ...
    def verify_authority(self):
        ...
        """
        try:
            if not self.karma.rpc.verify_authority(self.json()):
    ...
    def broadcast(self):
        ...
        if self.karma.nobroadcast:
            log.warning("Not broadcasting anything!")
            return self

        # Broadcast
        try:
            self.karma.rpc.broadcast_transaction(self.json(), api="network_broadcast")
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        self["blockchain"] = self.karma.rpc.chain_params
    ...
    def appendMissingSignatures(self):
        ...
        for pub in missing_signatures:
            wif = self.karma.wallet.getPrivateKeyForPublicKey(pub)
            if wif:
                self.appendWif(wif)
